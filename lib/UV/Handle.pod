...
L<active|http://docs.libuv.org/en/v1.x/handle.html#c.uv_is_active> returns
non-zero if the handle is active, zero if it's inactive. What "active" means
...
The L<loop|http://docs.libuv.org/en/v1.x/handle.html#c.uv_handle_t.loop> method
returns the L<UV::Loop> object this handle was initialized with.
...
The L<type|http://docs.libuv.org/en/v1.x/handle.html#c.uv_handle_t.type> method
returns the corresponding libuv handle type constant.
